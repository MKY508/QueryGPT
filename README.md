...
### 智能特性
- ✅ **自动环境检测**：WSL/Linux/macOS 自适应
- ✅ **WSL优化**：自动迁移到Linux文件系统，性能提升10倍
- ✅ **端口自动选择**：5000-5100范围内自动查找可用端口
- ✅ **网络问题处理**：自动配置国内镜像源
- ✅ **零配置**：自动创建所有必需的配置文件

### 后续使用
```bash
./start.sh   # 直接启动，无需其他操作
```

> **WSL用户提示**：脚本会自动将项目从 `/mnt/d/` 迁移到 `~/QueryGPT-github` 以获得最佳性能
